...
public func routes(_ router: Router, authentication: AuthenticationApiDelegate, dataModel: DataModelApiDelegate, formData: FormDataApiDelegate, headers: HeadersApiDelegate, multipleResponseCodes: MultipleResponseCodesApiDelegate, pathParsing: PathParsingApiDelegate, queryParameters: QueryParametersApiDelegate, authForSecurity1: Middleware) throws {
  ...
  routerForSecurity1.on(.GET, at: SwaggerPath(path: "/authentication/single")) { request -> Future<securityProtectedEndpointResponse> in
    return try authentication.securityProtectedEndpoint(with: request)
  }
  //for dataModel
