...
final class FormDataTests: XCTestCase {
  ...
  func testFormData() throws {
    ...
    let response = try app.sendRequest(to: "/form/request", method: .POST, body: input, bodyEncoding: .urlEncodedForm) as Response
    XCTAssertEqual(response.http.status, HTTPStatus.ok, "/form/request did not return a 200")
    XCTAssertNotNil(response.http.body.data, "/form/request should not be nil")
